...
cursor = db.clients.find({"address" : {'$regex' : ".*TX.*"}},{'_id':0, 'name':1, 'company':1, 'email':1, 'address':1})
	...
	with open('clients.csv','w') as outfile:
		...
		for client in entries:
				...
				try:
					...
					writer.writerow({"first":firstName, "last":lastName, "email":client["email"], "broker":client["company"],"zip":re.search('[0-9]{5},',client['address']).group(0)})
				except:
					print client
